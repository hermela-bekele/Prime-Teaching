...
type LessonPlanListCardProps = {
  session: SessionDto;
  selected: boolean;
  onSelect: (s: SessionDto) => void;
};

export function LessonPlanListCard({ session, selected, onSelect }: LessonPlanListCardProps) {
  ...
  const materialTag = primaryMaterialTag(getMaterials(session));
